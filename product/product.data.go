...
func searchForProductData(reportFilter ReportFilter) ([]Product, error) {
...
LOWER(productName)
FROM products WHERE `)
	if reportFilter.NameFilter != "" {
		queryBuilder.WriteString("productName LIKE ?")
		queryArgs = append(queryArgs, "%"+strings.ToLower(reportFilter.NameFilter)+"%")
	}
	if reportFilter.ManufacturerFilter != "" {
		queryBuilder.WriteString("manufacturer LIKE ?")
		queryArgs = append(queryArgs, "%"+strings.ToLower(reportFilter.ManufacturerFilter)+"%")
	}
	if reportFilter.SKUFilter != "" {
		queryBuilder.WriteString("sku LIKE ?")
		queryArgs = append(queryArgs, "%"+strings.ToLower(reportFilter.SKUFilter)+"%")
	}
